...
function gameRender() {
  ...
  site.insertAdjacentHTML(
    "beforeend",
    '<table class="table-secondary" id="table"></table>'
  );
  site.insertAdjacentHTML(
    "beforeend",
...
function getCellTemplate(i, j) {
  ...
  return `
        <td>
            <div class="fieldCell" style="background-color: ${getColor(
              i,
              j
            )};" data-id="${String(i) + String(j)}">${
    field[Number(i)][Number(j)].steps
  }
            </div>
        </td>
    `;
...
function getColor(i, j) {
  if (field[Number(i)][Number(j)].onPath) return "#DC0610";
  return field[Number(i)][Number(j)].canGo ? "#27AE60" : "#F1C40F";
}
